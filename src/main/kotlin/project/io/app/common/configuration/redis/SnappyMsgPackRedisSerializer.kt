...
class SnappyMsgPackRedisSerializer<T>(type: Class<T>) : RedisSerializer<T> {

    private val javaType: JavaType = ObjectMapper().typeFactory.constructType(type)
    ...
    override fun deserialize(bytes: ByteArray?): T? {
        ...
        return try {
            val uncompressedBytes = Snappy.uncompress(bytes)
            objectMapper.readValue(uncompressedBytes, javaType)
        } catch (ex: Exception) {
